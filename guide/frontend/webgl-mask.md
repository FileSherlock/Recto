...
5. **No fill.** A region is the kept component's own pixels. What a component encloses stays page: where the bars of adjacent lines touch they form one component, and the white gaps between them — with the punctuation standing there — must not be masked. (The recorded masks in `tests/golden/` were filled; no recorded page has such a gap, so they agree.)

### Edges — `MaskCore.transmission`

What the pages show (measured on a redacted corpus page, EFTA00173953): a redaction is a black rectangle with a soft rim of 1–3 pixels, all the way round, corners included. A rim pixel reads `page × t` with `t` constant along one side of one box. Boxes overlap into one region; where their rims cross, the `t` multiply (`bc · d3 / 255 = 9b`, to the digit); a box covers `ax · ay` of its own corner pixel. The boxes themselves cannot be told from the region's shape — but every straight stretch of its outline is a side of some box, and that is enough:
